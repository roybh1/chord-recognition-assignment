...
def get_song_chromagram(song_name: str):
    ...
    chromagram = convert_chromagram_to_dataframe(*get_chromagram_from_file(audio_file_path))
...
def run_training_pipeline(songs_names: list[str]):
    ...
    """
    chromagram = pd.concat([get_song_chromagram(song_name) for song_name in songs_names], ignore_index=True)
    ...
    initial_state_probs = filter_and_normalize_initial_probabilities(compute_initial_state_probabilities(), transition_matrix)
    ...
    return build_gaussian_hmm(initial_state_probs, transition_matrix, mu_array, states_cov_matrices), ix_2_chord

def get_predictions(h_markov_model: hmm.GaussianHMM, chord_dict: dict, song_name: str) -> pd.DataFrame:
